...
(function () {
  ...
  for (const el of inputElements) {
    ...
    // Only refresh results when the query text changes  -  not on Arrow keys (keyup would reset selection).
  ...
  // Set the active result by index. Keep focus on the input so Arrow/Enter keep working.
  function setActiveResult(index) {
    const { resultsElement, inputElement } = getActiveSearchElement();
    if (!resultsElement) return;

    const { result: activeResult } = getActiveResult();
    activeResult && activeResult.classList.remove('hextra-search-active');
    const result = resultsElement.querySelector(`a[data-index="${index}"]`);
    if (result) {
  ...
  function getResultsLength() {
    ...
    const n = parseInt(resultsElement.dataset.count || '0', 10);
  ...
  // Handle keyboard events (input stays focused; Enter opens highlighted or first result).
  function handleKeyDown(e) {
    const ctx = getActiveSearchElement();
    if (!ctx || !ctx.inputElement) return;
    const { inputElement, resultsElement } = ctx;

    const resultsLength = getResultsLength();
    const visible = resultsElement && !resultsElement.classList.contains('hx:hidden') && resultsLength > 0;
    const { result: activeResult, index: rawIndex } = getActiveResult();
    const activeIndex = Number.isFinite(rawIndex) && rawIndex >= 0 ? rawIndex : -1;
    ...
    switch (e.key) {
      case 'ArrowUp':
        ...
        else if (activeIndex < 0 && resultsLength > 0) setActiveResult(resultsLength - 1);
        break;
      case 'ArrowDown':
        if (!visible) return;
        e.preventDefault();
        if (activeIndex < 0) setActiveResult(0);
        else if (activeIndex + 1 < resultsLength) setActiveResult(activeIndex + 1);
        break;
      case 'Enter':
        if (!visible) return;
        e.preventDefault();
        {
          const toOpen = activeResult || resultsElement.querySelector('a[data-index="0"]');
          if (toOpen && toOpen.href) {
            window.location.assign(toOpen.href);
          }
          finishSearch();
        }
        break;
